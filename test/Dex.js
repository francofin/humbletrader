...
describe("DEX", () => {
let tokenSupply = setTokenSupply(10);
let token;
let owner;
let feeAccount;
let addr1;
...
before(async () => {
    ...
    let transferAction = await token.transfer(addr1.address, setTokenSupply("3"))
...
describe("Depositing Tokens", () => {
    ...
    describe("Success", () => {
        before(async() => {
            await token.connect(addr1).approve(dex.address, setTokenSupply("1"))
            tokenDeposited = await dex.connect(addr1).depositTokens(token.address, setTokenSupply("1"));
            result = await tokenDeposited.wait()
        })
        it("it should deposit tokens", async () => {  
            
            expect(tokenDeposited).to.changeTokenBalances(token, [addr1.address, dex.address], [setTokenSupply("-1"), setTokenSupply("1")]);
        })
    })
    ...
    describe("Failure", () => {
        it("it should fail to deposit tokens", async () => {
            ...
            await expect(dex.connect(addr2).depositTokens(token.address, setTokenSupply("1"))).to.be.reverted;
        })
    })
...
describe("Withdrawing Tokens", () => {
    ...
    describe("Success", () => {
        before(async() => {
            ...
            tokenWithdraw = await dex.connect(addr1).withdrawTokens(token.address, setTokenSupply(1));
        ...
        it("it should withdraw tokens", async () => {  
            ...
            console.log("Addr1 Balance", await token.balanceOf(addr1.address))
            console.log("Dex Balance", await token.balanceOf(dex.address))
            expect(tokenWithdraw).to.changeTokenBalances(token, [addr1.address, dex.address], [setTokenSupply(1), setTokenSupply(-1)]);
        })
    })
...
describe("Making Orders", () => {
    ...
    describe("Success", () => {
        before(async() => {
            ...
            depositTx = await dex.connect(addr1).depositTokens(token.address, setTokenSupply(1));
            depositResult = await depositTx.wait()
            orderTx = await dex.connect(addr1).makeOrder(altToken.address, token.address, setTokenSupply(1), setTokenSupply(1));
    ...
    describe("Failure", () => {
        it("SHould reject with no balance", async() => {
            await expect(dex.connect(addr1).makeOrder(token.address, altToken.address, setTokenSupply("1"), setTokenSupply("1"))).to.be.reverted;
        })
    })
...
describe("Order Actions", () => {
    ...
    before(async() => {
        ...
        tx = await altToken.connect(owner).transfer(addr2.address, setTokenSupply(3))
        result = await tx.wait(); 

        user2tx = await altToken.connect(addr2).approve(dex.address, setTokenSupply(2))
        user2Result = await user2tx.wait();

        dTx = await dex.connect(addr2).depositTokens(altToken.address, setTokenSupply(2));
    ...
    describe("Fill Order", () => {
        ...
        describe("Success", () => {
            it("Executes the trade and charge fees", async() => {
                ...
                expect(await dex.balanceOf(token.address, addr2.address)).to.equal(setTokenSupply("1"));
                ...
                // //Token Get
                expect(await dex.balanceOf(altToken.address, addr1.address)).to.equal(setTokenSupply("1"));
                expect(await dex.balanceOf(altToken.address, addr2.address)).to.equal(setTokenSupply("0.9"));
                expect(await dex.balanceOf(altToken.address, feeAccount.address)).to.equal(setTokenSupply("0.1"));
            })
            ...
            it('emits a Trade event', async () => {
                ...
                expect(args.amountGet).to.equal(setTokenSupply("1"))
                expect(args.tokenGive).to.equal(token.address)
                expect(args.amountGive).to.equal(setTokenSupply("1"))
